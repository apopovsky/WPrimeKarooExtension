...
class WPrimeCalculator(
    ...
    private fun updatePowerInternal(power: Double, timestamp: Long): Double {
        ...
        if (deltaTime <= EPSILON) return currentWPrime

        val oldWPrime = currentWPrime
        currentWPrime = when {
            power > criticalPower -> calculateDepletion(power, deltaTime, oldWPrime)
            power < criticalPower -> calculateRecovery(power, deltaTime, oldWPrime)
            else -> currentWPrime
        }
    ...
    private fun logSignificantChanges(power: Double, oldWPrime: Double, timestamp: Long) {
        val significantChange = abs(oldWPrime - currentWPrime) > (anaerobicCapacity * 0.05)
